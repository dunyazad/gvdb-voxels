...
void BitVolume::Initialize(dim3 dimensions, float voxelSize)
{
...
__device__ void BitVolume::setOccupied(BitVolumeInfo& info, dim3 voxelIndex)
{
	if (voxelIndex.x >= info.dimensions.x ||
		voxelIndex.y >= info.dimensions.y ||
		voxelIndex.z >= info.dimensions.z)
...
__device__ void BitVolume::setNotOccupied(BitVolumeInfo& info, dim3 voxelIndex)
{
	if (voxelIndex.x >= info.dimensions.x ||
		voxelIndex.y >= info.dimensions.y ||
		voxelIndex.z >= info.dimensions.z)
...
void BitVolume::OccupyFromPoints(float3 volumeMin, const float3* d_points, int numPoints)
{
...
void BitVolume::SerializeToFloat3(float3* d_output, unsigned int* d_outputCount)
{
	const int threads = 256;
...
__device__ float3 BitVolume::getEdgeVertexPosition(const BitVolumeInfo& info, dim3 base, int edge)
{
...
void BitVolume::MarchingCubes(std::vector<float3>& vertices, std::vector<int3>& faces)
{
	...
	Kernel_BitVolumeInfo_MarchingCubes << <blocks, threads >> > (
