...
__global__ void filter_points_shared_kernel(const float3* in_positions, float3* out_positions, int N, cuAABB aabb, int* global_count)
{
    // __shared__ 메모리 선언
    // flags: 점이 AABB 내에 있는지 여부 (1: 내부, 0: 외부)
    // block_scan_results: 블록 내 접두사 합(scan) 결과 저장
    __shared__ int flags[BLOCK_SIZE];
    __shared__ int block_scan_results[BLOCK_SIZE];
    __shared__ int block_offset; // 이 블록의 결과가 쓰여질 전역 메모리 시작 위치

    int tid = threadIdx.x;
    int gid = blockIdx.x * blockDim.x + threadIdx.x;

    // 1. 플래그 설정 (AABB 포함 여부 검사)
    if (gid < N) {
        float3 p = in_positions[gid];
        flags[tid] = (p.x >= aabb.min.x && p.x <= aabb.max.x &&
    ...
    __syncthreads(); // 모든 스레드가 플래그를 설정할 때까지 대기

    // 2. 블록 내 병렬 접두사 합 (Inclusive Scan)
    // Blelloch 스캔 알고리즘을 사용
    // a) Reduction (Up-sweep) phase
    for (int offset = 1; offset < BLOCK_SIZE; offset *= 2) {
        if (tid >= offset) {
            block_scan_results[tid] = flags[tid] + flags[tid - offset];
        }
        else {
            block_scan_results[tid] = flags[tid];
        }
        __syncthreads();
        // 결과를 다시 flags로 복사하여 다음 단계에서 사용
        flags[tid] = block_scan_results[tid];
        __syncthreads();
    }

    // b) Down-sweep phase (필요 시 Exclusive scan으로 변환. 여기서는 Inclusive로 충분)
    // Inclusive scan 결과가 block_scan_results에 최종 저장됨

    __syncthreads();

    // 3. 전역 오프셋 계산 (블록의 마지막 스레드만 실행)
    if (tid == BLOCK_SIZE - 1) {
        // 이 블록에서 찾은 총 점의 개수를 전역 카운터에 더하고, 이전 값을 받아옴
        int count_in_block = block_scan_results[tid];
        block_offset = atomicAdd(global_count, count_in_block);
    }

    __syncthreads(); // 모든 스레드가 block_offset을 공유할 때까지 대기

    // 4. 전역 메모리에 결과 쓰기
    if (gid < N) {
        // 현재 스레드의 점이 AABB 내에 있는 경우
        // (원래 flags 값으로 확인, 즉 scan 전 값이 1이었는지)
        if ((flags[tid] - (tid > 0 ? block_scan_results[tid - 1] : 0)) == 1) {
            // 자신의 출력 위치 계산 = 전역 오프셋 + 블록 내 상대 위치
            int out_index = block_offset + block_scan_results[tid] - 1;
            out_positions[out_index] = in_positions[gid];
        }
    }
}

__global__ void Kernel_CheckInside(
    const float3* inputPositions,
    unsigned int numberOfInputPositions,
    cuAABB aabb,
    float3* outputPositions,
    unsigned int* numberOfOutputPositions)
{
    extern __shared__ float3 s_positions[];

    __shared__ unsigned int s_localInsideCount;
    __shared__ unsigned int s_globalBaseIndex;
    ...
    if (0 == tid)
    {
        s_localInsideCount = 0;
    }
    __syncthreads();

    if(gid < numberOfInputPositions)
    {
        auto position = inputPositions[gid];
        if (aabb.contains(position))
        {
            unsigned int localIndex = atomicAdd(&s_localInsideCount, 1);
            if (localIndex < blockSize)
            {
                s_positions[localIndex] = position;
            }
        }
    }
    __syncthreads();

    unsigned int insideCount = min(s_localInsideCount, blockSize);

    if (0 == tid)
    {
        if (0 < insideCount)
        {
            s_globalBaseIndex = atomicAdd(numberOfOutputPositions, insideCount);
        }
    }
    __syncthreads();

    for (size_t i = tid; i < insideCount; i += blockSize)
    {
        float3 position = s_positions[i];

        outputPositions[s_globalBaseIndex + i] = position;
    }
}
...
void CUDAInstance::Test()
{
...

    {
        ...
        filter_points_shared_kernel << <gridSize, BLOCK_SIZE >> > (
            thrust::raw_pointer_cast(d_positions.data()),
            thrust::raw_pointer_cast(d_positions_inside.data()),
            N,
            aabb,
        ...
        //int count = d_count[0];
        //std::cout << "Points inside AABB (using Kernel): " << count << std::endl;

        //if (count > 0) {
        //    std::vector<float3> h_positions_inside(count);
        //    thrust::copy(d_positions_inside.begin(),
        //        d_positions_inside.begin() + count,
        //        h_positions_inside.begin());
...

    {
        ...
        unsigned int N = ply.GetPoints().size() / 3;
        thrust::host_vector<float3> h_positions(N);
        memcpy(h_positions.data(), ply.GetPoints().data(), sizeof(float3) * N);
        thrust::device_vector<float3> d_positions(h_positions);

        thrust::device_vector<float3> d_positions_inside(N);
        thrust::device_vector<unsigned int> d_numberOfOutputPositions(1, 0);
        ...
        size_t sharedMemSize = threadsPerBlock * sizeof(Eigen::Vector3f);

        Kernel_CheckInside << <blocksPerGrid, threadsPerBlock, sharedMemSize >> > (
            thrust::raw_pointer_cast(d_positions.data()),
            N,
            aabb,
            thrust::raw_pointer_cast(d_positions_inside.data()),
            thrust::raw_pointer_cast(d_numberOfOutputPositions.data()));

        CUDA_SYNC();

        CUDA_TE(NewKernel);

        int count = d_numberOfOutputPositions[0];
        std::cout << "Points inside AABB (using New Kernel): " << count << std::endl;
